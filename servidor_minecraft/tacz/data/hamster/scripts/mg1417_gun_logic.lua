...
local function getReloadTimingFromParam(param)
    ...
    local reload_1_feed = param.reload_1_feed * 1000
    local reload_1_cooldown = param.reload_1_cooldown * 1000
    local reload_2_feed = param.reload_2_feed * 1000
    local reload_2_cooldown = param.reload_2_cooldown * 1000
    ...
    if (reload_1_feed == nil or reload_1_cooldown == nil or reload_2_feed == nil or reload_2_cooldown == nil) then
        return nil
    end
    -- 顺序返回获取到的这 12 个参数
    return reload_feed, reload_cooldown, empty_feed, empty_cooldown, reload_1_feed, reload_1_cooldown, reload_2_feed, reload_2_cooldown
end
...
function M.tick_reload(api)
    ...
    local reload_feed, reload_cooldown, empty_feed, empty_cooldown, reload_1_feed, reload_1_cooldown, reload_2_feed, reload_2_cooldown = getReloadTimingFromParam(param)
    ...
    if (cache.is_tactical) then
        if (api:getAmmoAmount() > api:getMaxAmmoCount() * (2/3)) then
            -- 当换弹时间还不到战术装填时间时返回 FEEDING 和距离装填时间节点的剩余时间
            if (reload_time < reload_1_feed) then
                return TACTICAL_RELOAD_FEEDING, reload_1_feed - reload_time
            -- 当换弹时间达到了战术装填时间但是又没有完成整个流程时返回 FINISHING 和距离结束时间节点的剩余时间
            elseif (reload_time >= reload_1_feed and reload_time < reload_1_cooldown) then
                ...
                return TACTICAL_RELOAD_FINISHING, reload_1_cooldown - reload_time
            else
                -- 在以上两种情况之外（已经完成换弹）返回没有在装填，剩余时间置于 -1
                return NOT_RELOADING, -1
            end
        elseif (api:getAmmoAmount() > api:getMaxAmmoCount() / 3) then
            -- 当换弹时间还不到战术装填时间时返回 FEEDING 和距离装填时间节点的剩余时间
            if (reload_time < reload_2_feed) then
                return TACTICAL_RELOAD_FEEDING, reload_2_feed - reload_time
            -- 当换弹时间达到了战术装填时间但是又没有完成整个流程时返回 FINISHING 和距离结束时间节点的剩余时间
            elseif (reload_time >= reload_2_feed and reload_time < reload_2_cooldown) then
                -- 因为装填动作只进行一次而脚本却每一帧都在跑，所以需要一个标志位告诉我“装填”这一动作是否已经执行过了
                if (cache.reloaded ~= 1) then
                    -- 判断玩家装弹是否需要消耗弹药，约等于检测玩家是不是创造模式
                    if (api:isReloadingNeedConsumeAmmo()) then
                ...
                -- 在这个时间段要返回 FINISHING 和剩余时间
                return TACTICAL_RELOAD_FINISHING, reload_2_cooldown - reload_time
            else
                -- 在以上两种情况之外（已经完成换弹）返回没有在装填，剩余时间置于 -1
                return NOT_RELOADING, -1
            end
