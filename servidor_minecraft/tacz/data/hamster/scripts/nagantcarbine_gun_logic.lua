...
local function getReloadTimingFromParam(param)
    ...
    if (intro == nil or intro_fanning == nil or intro_empty == nil or loop == nil or ending == nil or ending_empty == nil or loop_feed == nil) then
        return nil
    end
    return intro, intro_fanning, intro_empty, loop, ending, ending_empty, loop_feed
end

function M.tick_reload(api)
    -- Get all timings from script parameter in gun data
    local param = api:getScriptParams();
    local intro, intro_fanning, intro_empty, loop, ending, ending_empty, loop_feed = getReloadTimingFromParam(param)
        ...
        if (api:getAmmoAmount() == api:getMaxAmmoCount()) then
            if (int_time >= ending_empty) then
                ...
                if (cache.is_tactical) then
                    return TACTICAL_RELOAD_FINISHING, ending_empty - int_time
                else
