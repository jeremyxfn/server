...
function M.reduceProgress(api)
    local cache = api:getCachedScriptData()
    if (cache ~= nil) then
        if(cache.progress ~= nil and cache.progress > 0.2) then
            cache.progress = cache.progress - 0.1
...
function M.shoot(api)
    ...
    if(cache.progress < 0.2) then
        api:safeAsyncTask(function ()
        return M.reduceProgress(api)
        end,1000, 1000, 10)
    end

    cache.progress = cache.progress + 0.5

    if(cache.progress >= shootDelay) then
        api:shootOnce(true)
        cache.progress = 0
    end
...
function M.tick_reload(api)
    ...
    -- Write back cache
    if (reloaded_count > cache.needed_count) then
        interrupted_time = api:getReloadTime() - loop
    end
