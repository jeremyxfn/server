...
local BULLET_TRACK = increment(blending_track_top)
...
function slide_states.normal.transition(this, context, input)
    if(context:isCrawl()) then
        context:runAnimation("bipod_start", context:getTrack(BLENDING_TRACK_LINE, SLIDE_TRACK), true, PLAY_ONCE_HOLD, 0.5)
...
function slide_states.slide.update(this, context)
    if (context:isStopped(context:getTrack(STATIC_TRACK_LINE, BASE_TRACK)) or context:isHolding(context:getTrack(BLENDING_TRACK_LINE, SLIDE_TRACK))) then
        context:runAnimation("bipod", context:getTrack(BLENDING_TRACK_LINE, SLIDE_TRACK), true, PLAY_ONCE_HOLD, 0.4)
    end
end
...
function slide_states.slide.transition(this, context, input)
    if(not context:isCrawl() or this.main_track_states.final.isfinal == 1) then
        context:runAnimation("bipod_end", context:getTrack(BLENDING_TRACK_LINE, SLIDE_TRACK), true, PLAY_ONCE_STOP, 0.2)
    ...
    if (not context:isStopped(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)) and context:isHolding(context:getTrack(BLENDING_TRACK_LINE, SLIDE_TRACK))) then
        return this.slide_states.slide_handdown
    end
end
...
function slide_states.slide_handdown.entry(this, context)
    context:runAnimation("bipod_handdown", context:getTrack(BLENDING_TRACK_LINE, SLIDE_TRACK), true, PLAY_ONCE_HOLD, 0.5)
end

function slide_states.slide_handdown.exit(this, context)
    if (this.main_track_states.final.isfinal ~= 1) then
        context:runAnimation("bipod_handup", context:getTrack(BLENDING_TRACK_LINE, SLIDE_TRACK), true, PLAY_ONCE_H0LD, 0.2)
    else
        context:stopAnimation(context:getTrack(BLENDING_TRACK_LINE, SLIDE_TRACK))
    end
end

function slide_states.slide_handdown.transition(this, context, input)
    if (context:isStopped(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)) or not context:isCrawl()) then
        return this.slide_states.slide
    end
end
...
function over_heat_states.normal.entry(this, context)
    context:runAnimation("over_heating_progress", context:getTrack(BLENDING_TRACK_LINE, OVER_HEATING_TRACK), true, PLAY_ONCE_STOP, 0)
    context:pauseAnimation(context:getTrack(BLENDING_TRACK_LINE, OVER_HEATING_TRACK))
    this.over_heat_states.normal.update(this, context)
end

-- 更新"不过热"状态
function over_heat_states.normal.update(this, context)
    if (context:getHeatProgress() >= 0.5) then
        context:setAnimationProgress(context:getTrack(BLENDING_TRACK_LINE, OVER_HEATING_TRACK), (context:getHeatProgress() - 0.5) * 2, true)
    end
...
function over_heat_states.over_heat.entry(this, context)
    ...
    context:runAnimation("static_over_heating", context:getTrack(BLENDING_TRACK_LINE, OVER_HEATING_TRACK), true, PLAY_ONCE_STOP, 0)
end
