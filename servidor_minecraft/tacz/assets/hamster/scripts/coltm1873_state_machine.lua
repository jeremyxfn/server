...
function main_track_states.start.transition(this, context, input)
    if (input == INPUT_DRAW) then
        if(context:getFireMode() == SEMI) then
            ...
            context:runAnimation("fanning", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0)
...
function main_track_states.fanning.transition(this, context, input)
    ...
    if (input == INPUT_INSPECT) then
        runFanningInspectAnimation(context)
...
function main_track_states.reload.entry(this, context)
    if (isNoAmmo(context)) then
        context:runAnimation(context:getFireMode() == SEMI and "reload_start" or "fanning_reload_start", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    else
        context:runAnimation(context:getFireMode() == SEMI and "reload_start" or "fanning_reload_start", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    end
...
function main_track_states.reload.transition(this, context, input)
    if (input == main_track_states.reload.retreat or input == INPUT_CANCEL_RELOAD) then
        context:runAnimation(context:getFireMode() == SEMI and "reload_end" or "fanning_reload_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        if (context:getFireMode() == SEMI) then
            return main_track_states.idle
        else
            return main_track_states.fanning
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (context:getFireMode() == AUTO) then
            ...
            -- context:runAnimation("cowboy_shoot", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        else
