...
function base_track_state.normal.transition(this, context, input)
    if(context:isCrawl() and context:isStopped(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK))) then
        return this.base_track_state.bipod
    end
end

function base_track_state.bipod.entry(this, context)
    context:runAnimation("bipod_start", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
end

function base_track_state.bipod.update(this, context)
    if (context:isHolding(context:getTrack(STATIC_TRACK_LINE, BASE_TRACK)) or context:isStopped(context:getTrack(STATIC_TRACK_LINE, BASE_TRACK))) then
        context:runAnimation("bipod", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, LOOP, 0)
    end
end

function base_track_state.bipod.transition(this, context, input)
    if(not context:isCrawl() and context:isStopped(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK))) then
        return this.base_track_state.normal
    end
end

function base_track_state.bipod.exit(this, context)
    context:runAnimation("bipod_end", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
end
