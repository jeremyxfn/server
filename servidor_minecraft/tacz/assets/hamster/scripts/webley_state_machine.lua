...
function main_track_states.reload.update(this, context)
    if (main_track_states.reload.loaded_ammo > main_track_states.reload.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if(context:isHolding(track)) then
            if(main_track_states.reload.emptyload == 1 or main_track_states.reload.vacancy > 0 or context:getMagExtentLevel() == 1) then
                if (context:getMagExtentLevel() == 1) then
        ...
        if(main_track_states.reload.timestamp ~= -1 and context:getCurrentTimestamp() > main_track_states.reload.timestamp + 0.8) then
...
function main_track_states.reload.transition(this, context, input)
    if (input == main_track_states.reload.retreat or input == INPUT_CANCEL_RELOAD) then
        if(main_track_states.reload.emptyload == 1) then
            context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), 0.42, false)
            context:runAnimation("reload_end_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.3)
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        context:runAnimation("hammer", track, true, PLAY_ONCE_STOP, 0)
    end
    return nil
end
