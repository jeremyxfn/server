...
function base_track_state.normal.transition(this, context)
    if(context:getFireMode() == SEMI) then
        ...
        return this.base_track_state.single
    end
end

function base_track_state.single.entry(this, context)
    this.base_track_state.mode = 1
    context:runAnimation("single_idle", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, LOOP, 0)
end

function base_track_state.single.transition(this, context, input)
    if(input == this.INPUT_HAMMER) then
...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_RELOAD) then
        if (this.base_track_state.mode == -1) then
            return this.main_track_states.reload
        else
            return this.main_track_states.singlereload
        end
    end
...
function main_track_states.reload.entry(this, context)
    if (isNoAmmo(context) or context:getMagExtentLevel() == 1) then
        ...
        context:runAnimation("double_reload_start_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.1)
    else
        context:runAnimation("double_reload_start", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    end
    if (context:getMagExtentLevel() == 1) then
        main_track_states.reload.need_ammo = context:getMaxAmmoCount()
    else
            ...
            if(main_track_states.reload.emptyload == 1 or main_track_states.reload.vacancy > 0) then
                if (context:getMagExtentLevel() == 1) then
                    if (main_track_states.reload.loaded_ammo == 0) then
                        context:runAnimation("double_reload_loader", track, false, PLAY_ONCE_HOLD, 0)
                    end
                    main_track_states.reload.vacancy = 0
                    main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 6
                else
                    if (main_track_states.reload.loaded_ammo ~= main_track_states.reload.need_ammo) then
                        context:runAnimation("double_reload_loop_empty", track, false, PLAY_ONCE_HOLD, 0)
                    end
                    ...
                    if(main_track_states.reload.emptyload == 1 and main_track_states.reload.vacancy == -1) then
                        main_track_states.reload.vacancy = 6
                    end
                    main_track_states.reload.timestamp = context:getCurrentTimestamp()
                end
            else
                context:runAnimation("double_reload_loop", track, false, PLAY_ONCE_HOLD, 0.2)
                main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 1
            end
        end
        if(main_track_states.reload.timestamp ~= -1 and context:getCurrentTimestamp() > main_track_states.reload.timestamp + 1.4) then
            main_track_states.reload.vacancy = main_track_states.reload.vacancy - 1
            main_track_states.reload.timestamp = -1
        end
    end
...
function main_track_states.reload.transition(this, context, input)
    if (input == main_track_states.reload.retreat or input == INPUT_CANCEL_RELOAD) then
        if(main_track_states.reload.emptyload == 1) then
            if (context:getMagExtentLevel() == 1) then
                context:runAnimation("double_reload_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.1)
            else
                context:runAnimation("double_reload_end_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            end
            main_track_states.reload.emptyload = 0
        else
            context:runAnimation("double_reload_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        end
...
function main_track_states.singlereload.entry(this, context)
    if (isNoAmmo(context) or context:getMagExtentLevel() == 1) then
        main_track_states.reload.vacancy = -1
        main_track_states.reload.emptyload = 1
        context:runAnimation("single_reload_start_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.1)
    else
        context:runAnimation("single_reload_start", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    end
    if (context:getMagExtentLevel() == 1) then
        main_track_states.reload.need_ammo = context:getMaxAmmoCount()
    else
...
function main_track_states.singlereload.update(this, context)
    if (main_track_states.reload.loaded_ammo > main_track_states.reload.need_ammo or not context:hasAmmoToConsume()) then
        context:trigger(main_track_states.reload.retreat)
    else
        local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
...
function main_track_states.singlereload.transition(this, context, input)
    if (input == main_track_states.reload.retreat or input == INPUT_CANCEL_RELOAD) then
        if(main_track_states.reload.emptyload == 1) then
            if (context:getMagExtentLevel() == 1) then
                context:runAnimation("single_reload_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            else
                context:runAnimation("single_reload_end_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.1)
            end
            main_track_states.reload.emptyload = 0
        else
            context:runAnimation("single_reload_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
    ...
    if (input == INPUT_SHOOT) then
        ...
        if (context:getFireMode() == BURST) then
            if (this.base_track_state.mode == -1) then
                ...
                context:runAnimation("shoot_single", track, true, PLAY_ONCE_STOP, 0)
            end
            context:trigger(this.INPUT_HAMMER)
        else
            context:runAnimation("shoot_single", track, true, PLAY_ONCE_STOP, 0)
            context:runAnimation("bolt", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        end
